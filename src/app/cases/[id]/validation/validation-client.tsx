    ...
    <div className="min-h-full">
      <div className="mx-auto max-w-6xl px-6 py-8">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-label text-muted-foreground">Data validation &amp; registration</p>
        ...
        <div className="mt-7 grid grid-cols-1 gap-8 lg:grid-cols-[340px_1fr]">
          ...
          <section aria-label="Series" className="space-y-4">
            ...
            <div className="rounded-md border border-border bg-card p-4">
              <h3 className="text-label text-muted-foreground">Registration confidence</h3>
              ...
              {confidence !== undefined && confidence < 0.85 && (
                <p className="mt-2 text-xs text-status-warn">Below 0.85 review threshold near vertex slices.</p>
              )}
            </div>
          </section>
          ...
          <section aria-label="Registration comparison" className="space-y-6">
            <div className="rounded-md border border-border bg-card p-5">
              ...
              <label className="mt-4 block">
                <span className="text-label text-muted-foreground">Scrub slice</span>
                <input
                  type="range"
                  min={20}
                  max={76}
            ...
            {kase.registrationWarnings.length > 0 && (
              <div className="rounded-md border border-warn/30 bg-warn-soft p-4" role="alert">
                <h3 className="flex items-center gap-2 text-sm font-medium text-status-warn">
                  <AlertTriangle className="h-4 w-4" aria-hidden /> Registration warnings
                </h3>
                <ul className="mt-2 space-y-1.5">
                  {kase.registrationWarnings.map((w) => (
                    <li key={w} className="text-[13px] leading-relaxed text-status-warn/90">{w}</li>
                  ))}
                </ul>
              </div>
            )}
              ...
              <p className="max-w-md text-[13px] leading-relaxed text-muted-foreground">
                {needsReview
                  ...
                  : kase.registrationDecision === "approved"
                    ? "Registration approved. You can proceed to contour review."
                    : "Review the overlay, then approve or send back."}
              </p>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => decide("return")}>
                  <Undo2 className="h-3.5 w-3.5" aria-hidden /> Return to import
                </Button>
                <Button variant="secondary" size="sm" onClick={() => decide("adjust")} disabled={kase.registrationDecision === "approved"}>
                  <ArrowLeftRight className="h-3.5 w-3.5" aria-hidden /> Adjust registration
                </Button>
  ...
  return (
    <div className="panel-edge rounded-md border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        ...
        <span className={"text-[11px] capitalize " + statusCls}>{s.status}</span>
      </div>
      ...
      <dl className="num mt-3 grid grid-cols-3 gap-2 text-[11px] text-muted-foreground">
        <div><dt className="sr-only">Slices</dt><dd>{s.sliceCount} slices</dd></div>
        <div><dt className="sr-only">Thickness</dt><dd>{s.sliceThicknessMm} mm</dd></div>
        <div><dt className="sr-only">Received</dt><dd>{new Date(s.receivedAt).toLocaleDateString("en-GB")}</dd></div>
      </dl>
    </div>
  );
}
